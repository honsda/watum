...
function getRefreshTokenCookieDeleteOptions(cookieName: string, url: URL) {
	if (cookieName === LEGACY_HOST_REFRESH_TOKEN_COOKIE_NAME) {
		return {
	...
	return getRefreshTokenCookieOptions(url);
}
...
function setRefreshTokenCookie(token: string) {
	...
	for (const cookieName of getRefreshTokenCookieNames(url)) {
		if (cookieName !== preferredCookieName) {
			cookies.delete(cookieName, getRefreshTokenCookieDeleteOptions(cookieName, url));
		}
	}
...
export async function refreshSession(): Promise<{
	...
	const result = await withTransaction<RefreshSessionResult>(async (connection) => {
		...
		if (tokenRow) {
			await getPool().query<ResultSetHeader>(
				`DELETE FROM refresh_tokens
				 WHERE token_hash = ? OR (user_id = ? AND context_binding = ?)`,
				[refreshTokenHash, tokenRow.user_id, contextBinding]
			);
		} else {
