...
const searchClassRoomsSchema = v.object({
	...
	classRoomType: v.optional(
		v.picklist(["REGULER", "LAB_KOMPUTER", "LAB_BAHASA", "AUDITORIUM"]),
	),
	minCapacity: v.optional(v.number()),
	maxCapacity: v.optional(v.number()),
});

export const searchClassRooms = query(
	searchClassRoomsSchema,
	async (filters) => {
		await requireRole(["ADMIN", "LECTURER", "STUDENT"]);
		...
		if (filters.classRoomType)
			where.push(["class_room_type", "=", filters.classRoomType]);
		...
		if (q) {
			...
			if (
				["REGULER", "LAB_KOMPUTER", "LAB_BAHASA", "AUDITORIUM"].includes(
					normalizedType,
				)
			) {
...
export const getClassRoomDashboardSummary = query(
	...
	async ({ timezone }) => {
		...
		const currentDay =
			dayOfWeek >= 1 && dayOfWeek <= DAY_ORDER.length
				...
				? Math.min(
						...
						Math.round(
							(totalOccupiedMinutes /
								(totalRooms * ROOM_METRIC_WINDOW_MINUTES)) *
								100,
						),
					)
				: 0;
		...
		const lowUtilizationThreshold = Math.round(
			ROOM_METRIC_WINDOW_MINUTES * 0.3,
		);
	...
	async ({ timezone, cursor, pageSize = DASHBOARD_PAGE_SIZE }) => {
		...
		for (const [offsetIdx, day] of rotatedDays.entries()) {
			if (currentDay && offsetIdx === 0) {
				...
				nextCaseParts.push(
					`WHEN s.day = ? THEN '${String(offsetIdx).padStart(2, "0")}'`,
				);
		...
		const [usageRows, currentRows, nextRows] = await Promise.all([
			pool.query(usageSql, baseValues).then(([rows]) => rows as RoomUsageRow[]),
			currentDay
				? pool
						.query(currentSql, [
							...baseValues,
			...
			pool
				.query(nextSql, [...baseValues, ...nextCaseValues])
				.then(([rows]) => rows as RoomNextKeyRow[]),
		]);

		const usageByRoomId = new Map(
			usageRows.map((row) => [row.class_room_id, row]),
		);
		...
		const nextByRoomId = new Map(
			nextRows
				.filter((row): row is RoomNextKeyRow & { next_key: string } =>
					Boolean(row.next_key),
				)
		...
		const courseNameByScheduleId = new Map(
			scheduleCourseRows.map((row) => [
				row.schedule_id,
				row.course_name ?? "Terjadwal",
			]),
		);
		...
		const items = roomRows.map((room) => {
			...
			const nextUseDay =
				(nextParts[1] as (typeof DAY_ORDER)[number] | undefined) ?? null;
			...
			const metric: RoomMetric = {
				...
				utilizationPercent: Math.round(
					(toNumber(usage?.occupied_minutes) / ROOM_METRIC_WINDOW_MINUTES) *
						100,
				),
...
export const getClassRoomUtilization = query(
	...
	async ({ classRoomId, timezone }) => {
		...
		const utilization: Record<
			string,
			Array<{ start: string; end: string; course: string }>
		> = {};
...
export const bulkUpdateClassRooms = form(
	...
	async (data) => {
		...
		const results: Array<{ id: string; ok: boolean; message?: string }> = [];
		for (const id of ids) {
			const [room] = await selectClassRooms(getPool(), {
				where: [["id", "=", id]],
			});
			if (!room) {
			...
			const classRoomType =
				data.classRoomType || room.class_room_type || "REGULER";
