...
import {
	...
	insertStudent,
	insertUser,
	type SelectGradesWhere,
	type SelectStudentsResult,
	type SelectStudentsWhere,
	selectGrades,
	selectStudents,
	selectStudyPrograms,
	selectUsers,
...
type StudentAccessFilters = {
	...
	facultyId?: string;
	facultyName?: string;
	minYearAdmitted?: number;
	maxYearAdmitted?: number;
};

async function selectLecturerStudents({
	lecturerId,
	limit,
	afterId,
	filters = {},
	...
	limit: number;
	afterId?: string;
	filters?: StudentAccessFilters;
	q?: string;
}) {
	const sqlParts = [
		"SELECT DISTINCT s.id, s.name, s.email, s.study_program_id, sp.name AS study_program_name, f.name AS faculty_name, s.year_admitted",
		"FROM students s",
	...
	if (q) {
		...
		const qWordPrefix = `% ${q}%`;
		sqlParts.push("AND (s.name LIKE ? OR s.name LIKE ?)");
		values.push(qPrefix, qWordPrefix);
		const cursor = decodeKeysetCursor(afterId, "students:name");
		if (cursor) {
			sqlParts.push("AND (s.name > ? OR (s.name = ? AND s.id > ?))");
			values.push(cursor.value, cursor.value, cursor.id);
		}
	} else if (afterId) {
		sqlParts.push("AND s.id > ?");
		values.push(afterId);
	}
	...
	if (filters.name) {
		...
		values.push(containsSearchPattern(filters.name));
	}
	if (filters.email) {
		sqlParts.push("AND s.email LIKE ?");
	...
	if (filters.studyProgramName) {
		sqlParts.push("AND MATCH(sp.name) AGAINST(? IN BOOLEAN MODE)");
	...
	if (filters.facultyName) {
		sqlParts.push("AND MATCH(f.name) AGAINST(? IN BOOLEAN MODE)");
		values.push(fulltextSearchPattern(filters.facultyName));
	}
	...
	sqlParts.push(q ? "ORDER BY s.name ASC, s.id ASC" : "ORDER BY s.id ASC");
	sqlParts.push("LIMIT ?");
	values.push(limit + 1);

	const [rows] = await getPool().query(sqlParts.join(" "), values);
	return rows as SelectStudentsResult[];
}

async function lecturerCanAccessStudent(lecturerId: string, studentId: string) {
...
export const searchStudents = query(searchStudentsSchema, async (filters) => {
	...
	if (filters.name)
		where.push(["name", "LIKE", containsSearchPattern(filters.name)!]);
	if (filters.email)
		where.push(["email", "LIKE", containsSearchPattern(filters.email)!]);
	if (filters.studyProgramId)
		where.push(["study_program_id", "=", filters.studyProgramId]);
	if (filters.studyProgramName)
		where.push([
			"study_program_name",
			"FULLTEXT",
	...
	if (filters.facultyName)
		where.push([
			"faculty_name",
			"FULLTEXT",
			fulltextSearchPattern(filters.facultyName)!,
		]);
	if (filters.minYearAdmitted != null && filters.maxYearAdmitted != null) {
		where.push([
			"year_admitted",
			"BETWEEN",
			filters.minYearAdmitted,
			filters.maxYearAdmitted,
		]);
...
export const getStudent = query(v.string(), async (id) => {
	...
	if (
		user.role === "LECTURER" &&
		!(await lecturerCanAccessStudent(user.lecturerId!, id))
	) {
		throw error(403, "Anda tidak berhak melihat data mahasiswa ini");
	}
...
export const getStudentGPA = query(v.string(), async (studentId) => {
	...
	if (
		user.role === "LECTURER" &&
		!(await lecturerCanAccessStudent(user.lecturerId!, studentId))
	) {
		throw error(403, "Anda tidak berhak melihat IPK mahasiswa ini");
	}
	const gradeWhere: SelectGradesWhere[] = [["student_id", "=", studentId]];
	if (user.role === "LECTURER") {
		gradeWhere.push(["lecturer_id", "=", user.lecturerId!]);
	}
...
export const updateStudent = form(
	...
	async (data) => {
		...
		if (!updateData.studyProgramId)
			throw error(400, "Program studi wajib dipilih");
		const [student] = await selectStudents(getPool(), {
			where: [["id", "=", id]],
		});
		if (!student) throw error(404, "Mahasiswa tidak ditemukan");
		...
		await withTransaction(async (conn) => {
			...
			const [linkedUser] = await selectUsers(conn, {
				where: [["student_id", "=", id]],
			});
			if (linkedUser?.id) {
				await updateUser(
...
export const deleteStudent = command(v.string(), async (id) => {
	...
	await withTransaction(async (conn) => {
		const [user] = await selectUsers(conn, {
			where: [["student_id", "=", id]],
		});
			...
			for (const id of ids) {
				...
				await updateStudentDb(
					conn,
					{
						...
						study_program_id:
							data.studyProgramId || student.study_program_id || "",
					},
					{ id },
				);
				results.push({ id, ok: true });
