...
import {
	...
	insertCourse,
	type SelectCoursesResult,
	type SelectCoursesWhere,
	selectCourses,
	selectLecturers,
	selectStudyPrograms,
...
export const getCourse = query(v.string(), async (id) => {
	...
	if (user.role === "LECTURER" && course.lecturer_id !== user.lecturerId) {
		throw error(403, "Anda tidak berhak melihat mata kuliah ini");
	}
	return course;
});
...
export const createCourse = form(courseCreateSchema, async (data) => {
	...
	const { id } = await insertWithGeneratedId({
		...
		readIds: async (connection) =>
			(await selectCourses(connection)).map((course) => course.id),
...
export const updateCourse = form(courseSchema, async (data) => {
	...
	const [existing] = await selectCourses(getPool(), {
		where: [["id", "=", data.id]],
	});
	if (!existing) {
		throw error(404, "mata kuliah tidak ditemukan");
...
export const bulkDeleteCourses = command(
	...
	async (idsParam) => {
		...
		if (ids.length > 200)
			throw error(400, "Maksimal 200 mata kuliah sekaligus");
		...
		for (const id of ids) {
			...
			await updateCourseDb(
				getPool(),
				{
					...
					study_program_id:
						data.studyProgramId || course.study_program_id || "",
