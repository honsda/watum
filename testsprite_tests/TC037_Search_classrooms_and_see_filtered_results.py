...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Ruang Kelas' (Classrooms) button (interactive element index 313) to open the Classrooms page.
        # button "Ruang Kelas"
        elem = page.locator("xpath=/html/body/div/div/aside/nav/section[2]/div[2]/button[2]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Type '000 TC022' into the classroom search input (index 1459), wait for the UI to update, and verify the matching room appears in the list.
        # text input aria-label="Cari ruang"
        elem = page.locator("xpath=/html/body/div/div[2]/main/div[2]/section/label/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("000 TC022")
