...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Ruang Kelas' (Classrooms) navigation button to open the classrooms list (element index 275).
        # button "Ruang Kelas"
        elem = page.locator("xpath=/html/body/div/div/aside/nav/section[2]/div[2]/button[2]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the classroom row for '000 TC029 Delete Room' to open its detail/actions (click element index 1440).
        # "000 TC029 Delete Room Reguler • kapasita..."
        elem = page.locator("xpath=/html/body/div/div/main/div[2]/section/div[2]/div[2]/div").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the 'Hapus' (Delete) button for 000 TC029 Delete Room (element index 3445) to open the confirmation dialog.
        # button "Hapus"
        elem = page.locator("xpath=/html/body/div/div/main/div[2]/section[2]/div/div[2]/button[2]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the confirmation button 'Ya, hapus ruang' (element index 3493) to perform the deletion.
        # button "Ya, hapus ruang"
        elem = page.locator("xpath=/html/body/div/div/main/div[2]/section[2]/section/div/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Close the right-side detail/drawer (the 'Tutup detail' button index 3443) so the classrooms list is fully visible, then verify whether '000 TC029' still appears.
