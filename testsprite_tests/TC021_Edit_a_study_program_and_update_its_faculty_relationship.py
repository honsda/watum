...
async def run_test():
    ...
    try:
        ...
        # -> Open the TC_FIX_SP program details by clicking the program row so the Edit button can be located.
        # "000 TestSprite Fixture Program TC_FIX_SP..."
        elem = page.locator("xpath=/html/body/div/div/main/div[2]/section/div[2]/div[8]/div").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the modal's 'Edit' button (interactive index 1728) to open the edit form for TC_FIX_SP.
        # button "Edit"
        elem = page.locator("xpath=/html/body/div/div/main/div[2]/section[2]/div/div[2]/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Open the Fakultas dropdown by clicking the faculty select element (index 1792) so the TC021 target faculty option can be selected.
        ...
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("Dr. TestSprite Fixture Updated")
