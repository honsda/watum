...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email and password fields with admin@watum.local / admin123 and submit the form.
        ...
        # -> Click the 'Fakultas' sidebar button (interactive element index 340) to open the Faculties list page.
        # button "Fakultas"
        elem = page.locator("xpath=/html/body/div/div/aside/nav/section[4]/div[2]/button[3]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Type 'TC030_FAC_DEL' into the faculty search input, wait for results to appear, then click the matching faculty row to open its form.
        # text input aria-label="Cari data fakultas"
        elem = page.locator("xpath=/html/body/div/div[2]/main/div[2]/section/label/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("TC030_FAC_DEL")
        
        # -> Type 'TC030_FAC_DEL' into the faculty search input, wait for results to appear, then click the matching faculty row to open its form.
        # "000 TC030 Delete Faculty TC030_FAC_DEL T..."
        elem = page.locator("xpath=/html/body/div/div/main/div[2]/section/div[2]/div/div").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the side-panel close button (index 1729) to close the faculty detail and then verify the faculty list shows the filtered result.
