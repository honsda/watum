...
async function runAdminWorker(loops, thinkTimeMs, rampUpDelayMs, collector, abortSignal) {
	...
	for (let i = 0; i < loops && !abortSignal.aborted; i++) {
		for (const cycle of WRITE_CYCLES) {
			...
			collector.add(
				`Update Course Temp ${cycle.courseId.slice(-2)}`,
				updateRes.status,
				updateRes.time
			);
			...
			collector.add(
				`Get Courses After Temp Update ${cycle.courseId.slice(-2)}`,
				verifyRes.status,
				verifyRes.time
			);
			...
			collector.add(
				`Restore Course ${cycle.courseId.slice(-2)}`,
				restoreRes.status,
				restoreRes.time
			);
		}
...
async function main() {
	...
	if (opts.admin) {
		workers.push(
			runAdminWorker(effectiveLoops, DEFAULT_WRITE_THINK_TIME, 0, collector, abortController.signal)
		);
	}
