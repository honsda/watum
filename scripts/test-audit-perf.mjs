...
async function test() {
  ...
  for (let i = 0; i < groupRows.length; i += BATCH_SIZE) {
    ...
    const conditions = batch
      .map(() => `(e.class_room_audit_sk = ? AND e.academic_year_start = ? AND e.semester_sort = ? AND e.schedule_day = ? AND TIME(e.schedule_start_time) = TIME(?) AND TIME(e.schedule_end_time) = TIME(?))`)
      .join(' OR ');
    const memberValues = batch.flatMap((g) => [
      g.resource_id, g.academic_year_start, g.semester_sort, g.day, g.start_time, g.end_time
    ]);
    ...
    console.log(`  Batch ${i / BATCH_SIZE + 1}: ${memberRows.length} rows in ${memberTime.toFixed(0)}ms`);
