...
const ADMIN_EMAIL = process.env.WATUM_ADMIN_EMAIL || 'admin@watum.local';
...
function request(path, method = 'GET', body = null, cookies = '') {
	return new Promise((resolve) => {
		...
		req.setTimeout(30000, () => {
			req.destroy();
			resolve({ status: 0, data: 'timeout', cookies: '' });
		});
...
async function main() {
	...
	const res = await request(
		`/_app/remote/k65086/getEnrollmentConflictAudit?payload=${encoded}`,
		'GET',
		null,
		cookies
	);
