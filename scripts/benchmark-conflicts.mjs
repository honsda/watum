	...
	request(url, method = 'GET', extraHeaders = {}, body = null) {
		return new Promise((resolve) => {
			...
			const headers = {
				...
				'User-Agent':
					'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/135.0.0.0 Safari/537.36 Bench',
...
async function login(client) {
	const res = await client.request(
		...
		{},
		{
			email: process.env.WATUM_ADMIN_EMAIL || 'admin@watum.local',
			password: getRequiredEnv('WATUM_ADMIN_PASSWORD')
		}
	);
	if (res.status !== 200) {
...
function formatStats(label, times, errors) {
	...
	console.log(
		`  ${label.padEnd(30)} n=${times.length.toString().padStart(4)} err=${errors.length.toString().padStart(2)} mean=${mean.toFixed(1).padStart(7)}ms med=${median.toFixed(1).padStart(7)}ms p95=${p95.toFixed(1).padStart(7)}ms p99=${p99.toFixed(1).padStart(7)}ms`
	);
}
...
async function benchmarkScenario(scenario, concurrency, durationMs) {
	...
	console.log(
		`  Cold cache:  ${coldRes.status === 200 ? 'OK' : 'FAIL'} ${coldRes.time.toFixed(1)}ms`
	);
	if (coldRes.status === 200 && coldRes.data?.summary) {
		const s = coldRes.data.summary;
		console.log(
			`    Groups: total=${s.totalGroups} room=${s.roomGroups} student=${s.studentGroups} lecturer=${s.lecturerGroups} enrollments=${s.conflictedEnrollments}`
		);
	}
	...
	console.log(
		`  Warm cache:  ${warmRes.status === 200 ? 'OK' : 'FAIL'} ${warmRes.time.toFixed(1)}ms`
	);
...
async function main() {
	...
	for (const [name, result] of Object.entries(allResults)) {
		if (result.times.length > 0) {
			...
			console.log(
				`${name.padEnd(25)} mean=${mean.toFixed(1).padStart(7)}ms p95=${p95.toFixed(1).padStart(7)}ms errors=${result.errors.length}`
			);
		}
	}
}
